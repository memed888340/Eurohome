...
export default function CreateAd({ onCancel }: CreateAdProps) {
  ...
  // Cloudinary-ə yükləmə funksiyası
  const uploadToCloudinary = async (files: File[]) => {
    const urls = [];
    const cloudName = "ddwoxaiol";
    const uploadPreset = "eurohome_preset"; // Cloudinary-də yaratdığın preset adı
  ...
  const handlePublishAd = async () => {
    ...
    try {
      // 1. Şəkilləri Cloudinary-ə yüklə
      const imageUrls = await uploadToCloudinary(selectedFiles);

      // 2. Firestore-a datanı yaz
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        <div className="space-y-6">
          ...
          <section className="bg-white rounded-2xl p-6 shadow-sm border space-y-6">
                ...
                {CATEGORIES.map((cat) => (
                  <button
                    ...
                    className={`px-2 py-2 text-[11px] rounded-xl border transition-all ${
                      selectedCatId === cat.id ? "border-orange-500 bg-orange-50 text-orange-600" : "border-gray-100 bg-gray-50 text-gray-600"
                    }`}
                  >
                    {cat.name}
                  </button>
                  ...
                  {activeCategoryData.subcategories.map((sub) => (
                    <button
                      ...
                      className={`px-4 py-2 text-xs rounded-full border transition-all ${
                        selectedSubCatId === sub.id ? "bg-black text-white" : "bg-white text-gray-500 hover:border-orange-400"
                      }`}
                    >
                      {sub.name}
                    </button>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={title} onChange={(e) => setTitle(e.target.value)} type="text" className="w-full border rounded-xl px-4 py-3 outline-none focus:border-orange-500" />
              </div>

              <div className="w-full sm:w-1/2">
                <label className="block text-sm font-semibold mb-1.5">Qiymət, AZN *</label>
                <input value={price} onChange={(e) => setPrice(e.target.value)} type="number" className="w-full border rounded-xl px-4 py-3 outline-none focus:border-orange-500" />
              </div>

              <div>
                <label className="block text-sm font-semibold mb-1.5">Təsvir *</label>
                <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={4} className="w-full border rounded-xl px-4 py-3 outline-none focus:border-orange-500 resize-none" />
              </div>
            </div>
          </section>
          ...
          <section className="bg-white rounded-2xl p-6 shadow-sm border space-y-4">
            <h2 className="text-lg font-bold mb-2 flex items-center gap-2"><MapPin size={20} className="text-orange-500" /> Əlaqə</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <select value={city} onChange={(e) => setCity(e.target.value)} className="w-full border rounded-xl px-4 py-3 bg-white">
                <option>Bakı</option><option>Sumqayıt</option><option>Gəncə</option><option>Xırdalan</option>
              </select>
              <input value={phone} onChange={(e) => setPhone(e.target.value)} type="tel" placeholder="050 000 00 00" className="w-full border rounded-xl px-4 py-3 outline-none focus:border-orange-500" />
            </div>
          </section>
          ...
          <div className="flex flex-col gap-3">
            <button 
              ...
              className={`w-full text-white font-bold py-4 rounded-2xl transition-all shadow-lg text-lg flex items-center justify-center gap-2 ${
                isPublishing ? "bg-gray-400 cursor-not-allowed" : "bg-orange-500 hover:bg-orange-600 shadow-orange-200 active:scale-[0.98]"
              }`}
            >
              {isPublishing ? (
                <>
            ...
            <button onClick={onCancel} className="w-full bg-white text-gray-600 font-semibold py-4 rounded-2xl border hover:bg-gray-50 transition-all">
              Ləğv et
            </button>
          </div>
        </div>
